...
#        inputs_grid_2 ,x_grid_on_2,x_grid_down_2,= feature_extractor.build(video_grid_2)  # [batch_size, input_length, hidden_size]  [B,75,512]
        x_grid1_on_50=tf.slice(x_grid_on_1,[0,0,0,0,0],[32,75,-1,-1,-1,])
        x_grid1_down_50=tf.slice(x_grid_down_1,[0,0,0,0,0],[-1,75,-1,-1,-1,])
        x_grid1 = tf.slice(video_grid_1,[0,0,0,0,0],[32,75,-1,-1,-1,])
        x_grid1_top = tf.slice(video_grid_1,[0,0,0,0,0],[16,75,-1,-1,-1,])
        x_grid1_last = tf.slice(video_grid_1,[0,0,0,0,0],[16,75,-1,-1,-1,])
...
#        x_grid1_down_50_32=tf.slice(x_grid_down_1,[16,0,0,0,0],[16,75,-1,-1,-1,])
        x_grid1_down_50_1=tf.slice(x_grid_down_1,[0,0,0,0,0],[16,75,-1,-1,-1,])
        x_grid1_down_50_2=tf.slice(x_grid_down_1,[16,0,0,0,0],[16,75,-1,-1,-1,])
        x_grid1_down_50_32 = tf.concat( [x_grid1_down_50_2, x_grid1_down_50_1], 0)        


#        x_grid2_on_50=tf.slice(x_grid_on_2,[0,0,0,],[-1,75,256,])
#        x_grid2_down_50=tf.slice(x_grid_down_2,[0,0,0,],[-1,75,256,])

    #    loss_down=tf.reduce_mean(tf.metrics.mean_absolute_error(x_a2_down_50,x_b2_down_50))
...
#        accuracy = tf.constant(0.0)
        ...
        con= tf.constant(1/29, shape=[29])
        loss_class_eu = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(fc2_softmax-con))))
    

        # train and eval
        labels_grid_1 = tf.squeeze(labels['label_grid_1'])  # [batch_size, ] 
        char_list_labels_grid_1 = label_util.string2char_list(labels_grid_1)  
        targets_grid_1 = self.preprocess_labels(labels_grid_1) 
#        logits_grid_1 = transformer(x_grid_down_1, inputs_unpadded_length, targets_grid_1)
      ...
      #    if params["ckpt_path"] != "":
            ...
            #    '/data/users/qunzhang/lipreading-master/*/model.ckpt-15000', assignment_map={"/": "/"})
            tf.train.init_from_checkpoint(
                '/data/users/qianxilu/lipreading_paper/ckpt_dis_grid_2/v3/model.ckpt-15000', assignment_map={"/": "/"})
