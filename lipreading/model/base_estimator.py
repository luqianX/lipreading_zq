...
from .cnn_extractor_dis_t_2 import LipNet
...
class BaseEstimator(object):
    ...
    def __init__(self, model_parms, run_config, **kwargs):
        ...
        self.estimator = tf.estimator.Estimator(
            self.model_fn, params=self.model_parms, config=self.run_config, **kwargs)
    ...
    def get_runConfig(model_dir,
        ...
        return tf.estimator.RunConfig(
            ...
            keep_checkpoint_max=40,            #keep_checkpoint_max：要保留的最近检查点文件的最大数量.当新文件被创建时,旧文件被删除.如果为None或0,则保留所有检查点文件.默认为5(也就是保留5个最近的检查点文件.)
