  ...
  """
    ...
    """
        ...
        self.embedding_softmax_layer = embedding_layer.EmbeddingSharedWeights(
            params["target_vocab_size"], params["hidden_size"], method="gather")#29
        self.encoder_stack = EncoderStack(params, train)
        self.decoder_stack = DecoderStack(params, train)

    def __call__(self, inputs, inputs_unpadded_length, targets=None):
    ...
    def encode(self, inputs, attention_bias, inputs_unpadded_length):
    ...
    """
        with tf.name_scope("encode"):
            ...
            with tf.name_scope("add_pos_encoding"):
                length = tf.shape(encoder_inputs)[1]#第1维是batch
                encoder_inputs += model_utils.get_position_encoding(
                    length, self.params["hidden_size"])#注意此处point-wise
    ...
    Args:
      targets: target values for the output sequence.
        int tensor with shape [batch_size, target_length](32,L)
      encoder_outputs: continuous representation of input sequence.
        float tensor with shape [batch_size, input_length, hidden_size](32,75,256)
      attention_bias: float tensor with shape [batch_size, 1, 1, input_length](32,1,1,75)
    ...
    """
        with tf.name_scope("decode"):
            ...
            decoder_inputs = self.embedding_softmax_layer(targets)#(32,L,256)
            with tf.name_scope("shift_targets"):
                ...
                decoder_inputs = tf.pad(decoder_inputs,
                                        [[0, 0], [1, 0], [0, 0]])[:, :-1, :]
            with tf.name_scope("add_pos_encoding"):
                length = tf.shape(decoder_inputs)[1]#L
            ...
            decoder_self_attention_bias = model_utils.get_decoder_self_attention_bias(
                length)
            #(32,L,256)<-(32,L,256);(32,75,256);(1,1,L,L);(32,1,1,75)
    ...
    def _get_symbols_to_logits_fn(self, max_decode_length):
      ...
      """
            ...
            self_attention_bias = decoder_self_attention_bias[:, :, i:i + 1, :
                                                              i + 1]
    ...
    def predict(self, encoder_outputs, encoder_decoder_attention_bias):
        ...
        cache = {
            ...
            for layer in range(self.params["num_hidden_layers"])#3
        }
    ...
    def __init__(self, params, train):
        ...
        for _ in range(params["num_hidden_layers"]):
            ...
            feed_forward_network = ffn_layer.FeedFowardNetwork(
                ...
                params["relu_dropout"], train, params["allow_ffn_pad"], use_conv=True, bidirection=True)
        ...
        for n, layer in enumerate(self.layers):
            ...
            with tf.variable_scope("layer_%d" % n):
                with tf.variable_scope("self_attention"):
                    ...
                    encoder_inputs = CausalConv1D(
                         self.hidden_size,
                         kernel_size=3,
                         strides=1,
                         dilation_rate=1,
                         activation=tf.nn.relu)(encoder_inputs)
  ...
  """
    ...
    def __init__(self, params, train):
        ...
        for _ in range(params["num_hidden_layers"]):
            ...
            feed_forward_network = ffn_layer.FeedFowardNetwork(
                params["hidden_size"], params["filter_size"],
                params["relu_dropout"], train, params["allow_ffn_pad"], use_conv=False, bidirection=False)
    ...
    Args:
      decoder_inputs: tensor with shape [batch_size, target_length, hidden_size](32,L,256)
      encoder_outputs: tensor with shape [batch_size, input_length, hidden_size](32,75,256)
      decoder_self_attention_bias: bias for decoder self-attention layer.
        [1, 1, target_len, target_length](1,1,L,L)
      attention_bias: bias for encoder-decoder attention layer.
        [batch_size, 1, 1, input_length](32,1,1,75)
