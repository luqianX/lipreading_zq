...
class EmbeddingSharedWeights(tf.layers.Layer):
    ...
    """
        ...
        if method not in ("gather", "matmul"):
            raise ValueError(
                "method {} must be 'gather' or 'matmul'".format(method))
        self.method = method
    ...
    """
        with tf.name_scope("embedding"):
            ...
            #(32,L)<-(32,L)mask中只有1和0
            mask = tf.to_float(tf.not_equal(x, 0))

            if self.method == "gather":
                #(32,L,256)<-(29,256),(32,L)
                embeddings = tf.gather(self.shared_weights, x)
                embeddings *= tf.expand_dims(mask, -1)
            else:  # matmul
                raise NotImplementedError('only gather method is implemented')
    ...
    Args:
      x: A float32 tensor with shape [batch_size, length, hidden_size](32,L,256)
    Returns:
      float32 tensor with shape [batch_size, length, vocab_size].
    """
        with tf.name_scope("presoftmax_linear"):
            batch_size = tf.shape(x)[0]#32
            length = tf.shape(x)[1]#L

            x = tf.reshape(x, [-1, self.hidden_size])#(32*L,256)
            #(32*L,29)<-(32*L,256);(29,256)
