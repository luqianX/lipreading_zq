...
def _pad_tensors_to_same_length(x, y):
  ...
  with tf.name_scope("pad_to_same_length"):
    x_length = tf.shape(x)[1]#L
    y_length = tf.shape(y)[1]#L
...
def padded_cross_entropy_loss(logits, labels, smoothing, vocab_size):
  ...
  with tf.name_scope("loss", values=[logits, labels]):
    ...
    with tf.name_scope("smoothing_cross_entropy", values=[logits, labels]):
      confidence = 1.0 - smoothing#int:0.8
      low_confidence = (1.0 - confidence) / tf.to_float(vocab_size - 1)#0.0074074077
      #(注意此处独热编码并非1和0而是0.8和low_confidence)
      soft_targets = tf.one_hot(
          tf.cast(labels, tf.int32),
          depth=vocab_size,#29
          on_value=confidence,
          off_value=low_confidence)
      #(32,L)<-(32,L,29);(32,L,29)注意logits不能进行缩放
    ...
    #以tf.cast替代，原来为（32，L)的张量,现在将不为0的全部替换为1
