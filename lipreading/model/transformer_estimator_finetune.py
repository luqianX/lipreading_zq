...
#        fc2,fc2_softmax=classfi.build(x_grid1_down_50)
        with tf.variable_scope("finetune_layers"):
            ...
            drop1 = tf.get_default_graph().get_tensor_by_name("Classfication/fc1/BiasAdd:0")
             # 将张量向新层传递
            out=keras.layers.Dense( 29,name='fc2',
                                        trainable=self.training,
                                        )(drop1)
            #(32,29)
            softmax=keras.layers.Activation('softmax')(out)
        loss_class_1=tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=out, labels=class_a))
        accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(softmax, -1), tf.argmax(class_a, -1)), tf.float32))
        '''   
...
#        logits_grid_1 = transformer(x_grid_down_1, inputs_unpadded_length, targets_grid_1)
        ...
        var_to_restore = [val for val in var if 'finetune_layers' not in val.name]
        ...
        #    train_op, metric_dict = get_train_op_and_metrics(loss_lip,loss_res,loss_class ,params)
            ...
            with tf.Session() as sess:
                saved_model='/data/users/qianxilu/lipreading_paper/data/ckpt/model.ckpt-10000'
                saver.restore(sess=sess, save_path= saved_model)
                var_to_init = [val for val in var if 'finetune_layers' in val.name] 
