...
DICT = list(string.ascii_lowercase) + [' ', '_']#['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', ' ', '_']
...
def string2indices(s, dic=DICT):
    ...
    string2index_table = tf.contrib.lookup.index_table_from_tensor(
        dic, num_oov_buckets=1, default_value=-1)#查找表
...
def string2char_list(string):
    ...
    """
    return tf.string_split(string, delimiter='')#注意delimiter
