...
import sys
sys.path.append("/data/users/qianxilu/lip_reading_paper_mixed/")
from lipreading.dataset.grid_dataset_dis_2 import grid_tfrecord_input_fn
from lipreading.dataset.grid_dataset_dis_2_45 import grid_tfrecord_input_fn_45
...
def main():
    ...
    train_file = os.path.join(args.data_dir,
                            "/data/users/qianxilu/lipreading_paper/data/tfrecord/unseen_train_vac_45.tfrecord".format(args.type))

    test_file = os.path.join(args.data_dir,
                              "/data/users/qianxilu/lipreading_paper/data/tfrecord/unseen_test_vac_7.tfrecord".format(args.type))
    ...
    #由有形参的函数变为无形参的函数，这样在下面用的时候更美观
    train_input_fn = lambda: grid_tfrecord_input_fn_45(mode=tf.estimator.ModeKeys.TRAIN, **train_input_params)
    eval_input_fn = lambda: grid_tfrecord_input_fn_45(mode=tf.estimator.ModeKeys.EVAL, **eval_input_params)
